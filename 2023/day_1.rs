fn main() {
    // My first ever program written in Rust!
    ...
    let puzzle_substrings: Vec<&str> = puzzle_input.split_whitespace().collect();
    let mut number_collector: Vec<i32> = Vec::new();
    
    for substring in puzzle_substrings {
        let letters: Vec<char> = substring.chars().collect();
        let mut numbers: Vec<i32> = Vec::new();

        for letter in letters {
            let parsed_letter: Result<i32, _> = letter.to_string().parse();
        ...
        if numbers.len() == 1 {
            let duplicated_number: Result<i32, _> = format!("{}{}", numbers[0], numbers[0]).parse();

            match duplicated_number {
                Ok(number) => {
                    number_collector.push(number)
                }
                Err(_) => ()
            }
        } else {
            ...
            let combined_number: Result<i32, _> = format!("{}{}", first_number, last_number).parse();

            match combined_number {
                Ok(number) => {
                    number_collector.push(number);
                }
                Err(_) => ()
    ...
    let sum: i32 = number_collector.iter().sum();
